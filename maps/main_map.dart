...
class _MapScreenState extends State<MapScreen> {
  ...
  final String userId = 'user3'; // Hardcoded user ID
  ...
  Future<void> toggleFavorite(String locationId) async {
    try {
      if (favorites.contains(locationId)) {
        await _wishlistService.removeFromWishlist(userId, locationId);
        setState(() {
          favorites.remove(locationId);
        });
      } else {
        await _wishlistService.addToWishlist(userId, locationId);
        setState(() {
  ...
  Future<void> toggleVisited(String locationId) async {
    try {
      if (visited.contains(locationId)) {
        await _visitedService.removeFromVisited(userId, locationId);
        setState(() {
          visited.remove(locationId);
        });
      } else {
        await _visitedService.addToVisited(userId, locationId);
        setState(() {
